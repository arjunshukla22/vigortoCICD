...
class VCXServicesClass: NSObject {
    ...
    class func createRoom(completion:@escaping (EnxRoomInfoModel) -> ()){
        ...
        let url = URL(string: "\(APIConstants.mainUrl)Api/EnableX_CreateToken")!
    ...
    class func fetchRoomInfoWithRoomId(roomId : String , completion:@escaping (EnxRoomInfoModel) -> ()){
        let url = URL(string: "\(APIConstants.mainUrl)Api/GetRoomInfo?RoomId=\(roomId)")!
    ...
    class func featchToken(requestParam : [String: String] , completion:@escaping (String) -> ()){
        //create the url with URL
//        let url = URL(string: "http://dtest.smileindia.com/Api/EnableX_CreateToken")!
//        //Create A session Object
//        let session = URLSession.shared
//        //Now create the URLRequest object using the url object
//        var request = URLRequest(url: url)
...
//        tast.resume()
         ...
         WebService.enableX_CreateToken(queryItems: requestParam) { (result) in
                 DispatchQueue.main.async {
                     switch result {
                     case .success(let response):
                        print(response.resultString ?? "")
                        completion(response.resultString ?? "")
