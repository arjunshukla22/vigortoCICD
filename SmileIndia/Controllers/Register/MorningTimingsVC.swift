...
class MorningTimingsVC: UIViewController , UIPickerViewDelegate, UIPickerViewDataSource, UITextFieldDelegate ,CLLocationManagerDelegate,MKMapViewDelegate{
    ...
    @IBAction func didTapNext(_ sender: UIButton) {
        ...
        if isValid()
        {
            ...
            NavigationHandler.pushTo(.eveningTimingsVC(businessHours, dict, array, image, customerType))

        }else
        {
            AlertManager.showAlert(type: .custom("Morning end timings cannot be lower than or similar to morning start timings."))
        }
    }
